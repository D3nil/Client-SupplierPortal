...
export class CreateTempFornitoreComponent implements OnInit {
  ...
  public saveOrUpdate(): void {
    this.isValid = this.createTempFornitoreService.validate(
      this.fornitoreTempor
    );
    if (this.isValid) {
      this.createTempFornitoreService
        .saveOrUpdate(this.fornitoreTempor)
        .subscribe(res => {
